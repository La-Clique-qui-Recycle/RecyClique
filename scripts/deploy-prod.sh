...
if $COMPOSE_CMD --help 2>/dev/null | grep -q -- "--env-file"; then
  ...
  $COMPOSE_CMD -f docker-compose.prod.yml -p recyclic-prod --env-file .env.production --env-file .build-meta.env up -d --remove-orphans

  # 4. Activer le service de backup automatique (Story B46-P4)
  echo "📦 Activation du service de backup automatique..."
  if [ -f "docker-compose.backup.yml" ]; then
    $COMPOSE_CMD -f docker-compose.backup.yml -p recyclic-prod --env-file .env.production --profile backup up -d postgres-backup || echo "⚠️  Service backup non démarré (peut nécessiter configuration)"
  else
